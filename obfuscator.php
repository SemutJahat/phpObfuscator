...
class PHPEncryptor {
	...
	public function Help(){
		...
		$logo .= "Coded by @unix666 (github.com/da7Q9RnPjm)\n";
		$logo .= "Usage: php thisfile.php input.php output.php\n\n";
		$logo .= "NOTICE:\n";
		$logo .= "- Output file will bi saved at output/ directory.\n\n";
		echo $logo;
	}
...
$tulis = fopen("output/".$output,'w+');
$save = fwrite($tulis,$hasil);
if($save) {
	echo "[+] Encrypted File Successfully Saved : output/".$output."\n";
} else {
	echo "[-] Encrypted File Failed to Save\n";
}
fclose($tulis);
$sizeFile2 = round(filesize("output/".$output)/1024,2);
